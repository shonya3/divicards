import { StashTab, IItem } from './types';
import { CardNameAmount, League, DivinationCardsSample } from '../types';
import { command } from '../command';
export const isCard = (name: string): boolean => {
	return cards.includes(name);
};

export const itemIsCard = (item: IItem): boolean => {
	return isCard(item.baseType);
};

export const cardsFromTab = (tab: StashTab): CardNameAmount[] => {
	const cards: CardNameAmount[] = [];
	try {
		if (tab.items === undefined) return cards;
