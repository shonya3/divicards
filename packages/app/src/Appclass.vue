...
const fileCardsStore = ref(new FileCards());
...
const onDrop = async (e: DragEvent) => {
	...
	// 	await fileCardsStore.value.addFromFile(file);
	// }
	await Promise.all(
		Array.from(e.dataTransfer?.files ?? []).map(file => {
			return fileCardsStore.value.addFromFile(file);
		})
	);
	fileCardsStore.value.fileCards = Array.from(fileCardsStore.value.fileCards);
};
		...
		<Transition>
			<div ref="filesTemplateRef" class="files" v-show="fileCardsStore.fileCards.length">
				<FileCard
					...
					v-bind="fileCard"
					@delete="id => fileCardsStore.deleteFile(fileCard.uuid)"
